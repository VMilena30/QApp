...
TEXTOS_OPT = {
    "pt": {
        ...
        "info1": (
            ...
            "QUBO (Quadratic Unconstrained Binary Optimization) e resolve essa formulação "
            "utilizando algoritmos quânticos variacionais (QAOA ou VQE), conforme selecionado."
        ),
        ...
        "info2": (
            ...
            "### Passo C — Escolha do algoritmo (por que existem duas opções?)\n\n"
            "**QAOA**\n"
            "- Indicado para problemas combinatórios em QUBO.\n"
            "- Controlado pelo número de camadas (profundidade).\n"
            "- Estrutura mais direta para otimização discreta.\n\n"
            "**VQE**\n"
            "- Mais flexível e dependente da escolha do ansatz.\n"
            "- Requer configuração de portas de rotação e emaranhamento.\n\n"
            "Em termos práticos:\n"
            "- QAOA é geralmente a escolha mais direta para QUBO.\n"
            "- VQE permite maior customização do circuito."
        ),
    
        "info21": (
            "### Observação sobre Upload (.txt)\n\n"
            "Ao utilizar arquivo, mantenha o formato exigido pela plataforma. "
            "Essa opção é indicada para instâncias maiores ou estudos comparativos.\n\n"
            "Caso ocorra erro de leitura, revise:\n"
        ...
        "info3": (
            "### Otimizador Clássico\n\n"
            "- **COBYLA**: método de otimização sem uso de derivadas, adequado para problemas de menor dimensão.\n"
            "- **SPSA**: método estocástico que estima gradientes com poucas avaliações, sendo mais robusto a ruído.\n\n"
            ...
            "- Mais camadas podem melhorar a qualidade da solução, mas também aumentam o custo computacional e a sensibilidade a ruído.\n\n"
            "### Número de Iterações\n\n"
            "- Define quantas atualizações dos parâmetros o otimizador vai fazer.\n"
            "- Poucas iterações podem não dar tempo suficiente para o modelo convergir para a melhor solução, enquanto muitas aumentam o tempo de execução."
        ),
    
        "info4_titulo": "4) Ajustes avançados: quando faz sentido alterar parâmetros?",
    
        "info4": (
            "### Variação de Inicialização\n\n"
            "Pode ser útil quando:\n"
            "- Há grande variação entre execuções.\n"
            "- O algoritmo parece ficar preso em mínimos locais.\n\n"
            "### Aumento de Profundidade\n\n"
            "Pode ser útil quando:\n"
            "- A solução encontrada é estável, mas de baixa qualidade.\n"
            "- O problema tem muitas variáveis e combinações possíveis, sendo mais difícil de resolver.\n\n"
            "Importante:\n"
            "- Em hardware real, circuitos mais profundos podem piorar os resultados devido ao ruído.\n"
            "- Recomenda-se testar profundidades crescentes de forma gradual."
        ),
    
        "info5_titulo": "5) O que devo analisar nos resultados?",
        ...
        "info5": (
            ...
            "- Verifique primeiro se a solução respeita o custo máximo.\n"
            "- Compare execuções com diferentes inicializações.\n"
            "- Se houver muita variação entre execuções (instabilidade nos resultados), aumente os shots, reduza a profundidade ou ambos.\n"
            "- Se a solução não melhorar ao longo das iterações dentro de uma execução (estagnação da otimização), aumente o número de iterações, teste outro otimizador ou ambos."
        ),
...
TEXTOS_ML = {
    "pt": {
        ...
        "selecione_features": "Selecione as features a serem extraídas da base",
        ...
        "help_1": "Escolha uma base de dados própria com sinais de vibração ou a base de dados da CWRU.",
    ...
    "en": {
        ...
        "selecione_features": "Select the features to extract from the dataset",
        ...
        "help_1": "Choose your own database with vibration signals or the CWRU dataset.",
...
TEXTOS_INF = {
    "pt": {
        ...
        **Referência:** Borujeni et al., *Quantum circuit representation of Bayesian networks*, Expert Systems With Applications (2021).
        ...
        "aa_k_help": "Número de iterações de AA. k maior pode r, mas também pode piorar se passar do ponto.",
        ...
        "warn_prob_sum": "A soma atual das probabilidades é",
        "warn_prob_sum_fix": "Para um nó raiz, a soma deve ser 1.",
    ...
    "en": {
        ...
        **Reference:** Borujeni et al., *Quantum circuit representation of Bayesian networks*, Expert Systems With Applications (2021).
        ...
        "warn_prob_sum": "The current sum of probabilities is",
        "warn_prob_sum_fix": "For a root node, the sum must be 1.",
        ...
        "info2": (
            ...
            "3. Fill in:\n"
            "   - Marginal probabilities (root node) if the node has no parents\n"
            "   - If the node has parents: enter a Conditional Probability Table (CPT)..\n\n"
...
def main():
    ...
    if st.session_state.step == "login":
        ...
        with right:
            ...
            /* Pequeno ajuste para colar no texto de cima (bem leve) */
            div[role="radiogroup"]{
    ...
    if st.session_state['pagina'] == 'inicio':
        ...
        st.header(textos_otim["info5_titulo"])
        st.write(textos_otim["info5"])
                    ...
                    if example_locked:
                        ...
                        if len(parents_now) == 0:
                            ...
                            edited = st.data_editor(
                                ...
                                hide_index=True,
                                num_rows="fixed",
                                key=f"qbn_root_probs_{nsel}",
                                column_config={
                                    "state": st.column_config.TextColumn(disabled=True),
                                    "prob": st.column_config.NumberColumn(format="%.6f"),
                                },
                            )
                            
                            probs = [float(x) if x is not None else 0.0 for x in edited["prob"].tolist()]
                            info["cpt"] = {(): probs}
                            
                            row_sum = sum(probs)
                            if abs(row_sum - 1.0) > 1e-9:
                                st.warning(
                                    f"{textos_inf['warn_prob_sum']} {row_sum:.6f}. {textos_inf['warn_prob_sum_fix']}"
                                )
